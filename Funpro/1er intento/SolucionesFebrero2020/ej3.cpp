...
typedef array<Persona, MAX_PERSONAS> APersonas;
struct Datos {
	int nelms = 0;
	APersonas elm;
};
...
void movimientos(Datos& mov)
{
	...
	while ( ! todos_cero(mov) ) {
		...
		assert(mov.elm[imayor].gastos > 0);
		assert(mov.elm[imenor].gastos < 0);
		double dd = menor(vabs(mov.elm[imayor].gastos),
						  vabs(mov.elm[imenor].gastos));
		cout << mov.elm[imenor].nombre << " paga " << dd
			 << " a " << mov.elm[imayor].nombre << endl;
...
void mostrar_dif(const Datos& dif)
{
	cout << endl;
	for (int i = 0; i < dif.nelms; ++i) {

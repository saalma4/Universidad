...
int main()
{
	...
	if (primera != "FIN") {
        ...
        cin >> pal;
        while (pal != "FIN") {
            if (sumaASCII(pal) < sumaASCIIPrimera) {
                pos = posicion(pal,datos); // pos contendra la posicion de la palabra si esta o la posicion donde deberia insertarse
                if (!(pos < datos.nPal && pal == datos.pal[pos])) { // no esta la palabra

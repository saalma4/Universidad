...
controller.listarPeliculas = async function(req, res, next){
    try{
        const filter = {}
        if(req.query.filtro === "titulo"){
            filter.order=[["titulo", "ASC"]]
        }else if(req.query.filtro === "anyo"){
            filter.order=[["anyo", "ASC"]]
        }
        const peliculas = await models.Pelicula.findAll(filter)
        res.render("index", {peliculas})
    }catch (error){
        res.send("Se ha producido un error " + error);
    }
}
controller.listarFrases = async function(req, res, next){
    try{
        const frases = await models.Frase_Celebre.findAll({
            include:[{
        ...
        res.render("frases", {frases, pelicula})
    }catch(error){
        res.send("Se ha producido un error " + error);
    }
}

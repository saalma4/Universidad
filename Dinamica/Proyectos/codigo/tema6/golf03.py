...
############## Sobreescribimos la función separate de la colision
############## palo-bola para detectar que la bola ha salido

global BOLA_LANZADA
BOLA_LANZADA=False

#--------------------------------
def bola_golpeada(arbiter, space, data):
	#simplemente pone a True la variable
	#global que nos dice que la bola ha sido lanzada
	global BOLA_LANZADA
	print('Bola lanzada')
	BOLA_LANZADA=True
	#return True
...
def run():
	...
	#agregamos el handler para detectar que la bola ha sido golpeada
	space.on_collision(1,2,separate=bola_golpeada)
...
#A) ponemos el suelo con un agujero (dos trozos de suelo)
	...
	suelo1 = pymunk.Segment(suelo_body,xy2pixel([0,0]) , xy2pixel([HOYO_m-RADIO_m*5,0]), 0)
	# Segmento del green DESPUÉS del hoyo
	suelo2 = pymunk.Segment(suelo_body,xy2pixel([HOYO_m+RADIO_m*5,0]) , [WIDTH,NIVEL_DEL_SUELO], 0)
	...
	INICIO_GREEN=WIDTH*16.68/20
	FIN_GREEN=WIDTH*19.68/20
...
#C) Creamos la cabeza del palo
	...
	alpha=lofts[num_palo]*np.pi/180
	h=0.06*PX_M*30
	w=0.02*PX_M*30
	p1 = Vec2d(0, 0)
	p2 = p1 + Vec2d(-h * np.tan(alpha), -h) 
	p3 = p2 + Vec2d(-w, 0)
	p4 = Vec2d(-(h * np.tan(alpha0) + w), 0)
	...
	#palo.velocity=Vec2d(42.8*PX_M,0)
	palo.velocity=Vec2d(28*PX_M,0)
	palo.angular_velocity=0
	#Palo 9, con 45 km/h hoyo en uno, Ace (sin magnus)	
	...
	SUELO_CORREGIDO=NIVEL_DEL_SUELO-RADIO_PX
	...
	while running:
		...
		for _ in range(substep):
			...
			if BOLA_LANZADA:
				if bola.position.y>=SUELO_CORREGIDO: #la bola está rodando
					if bola.position.x>INICIO_GREEN and bola.position.x<FIN_GREEN:
						suelo1.friction = 0.8   
						suelo2.friction = 0.8    
						Crr=0.05
					else:
						...
						suelo2.friction = 0.7   
						Crr=0.2	
					roz.aplicar_rodadura(bola,Crr=Crr,dt=dt)
					print('en el suelo',bola.angular_velocity)
				else:  # la bola está en el aire
					bola.mass=0.0001000
					v=bola.velocity.length*M_PX
					#bola.angular_velocity*=0.99
					Cd=roz.get_Cd(v,2*RADIO_m,golf=True)
					roz.aplicar_newton(bola,np.pi*RADIO_m**2, M_PX=M_PX, Cd=Cd)
					#f_mag=roz.aplicar_magnus(bola,np.pi*RADIO_m**2, M_PX=M_PX, k=1.3)
					roz.aplicar_frenado_aire_rotacional(bola, Cm=0.07)
					
					#f_mag*=(M_PX/9.81*1000)
					print(bola.position.x)
					#print(f'{bola.angular_velocity:.2f}  ({f_mag.x:.2f},{f_mag.y:.2f})')
		...
		#----- pinto el palo hasta que impacta (casi no se ve)----
		if bola.velocity.length<0.1:
			vertices_mod = []
			for v in palo_shape.get_vertices():
				# Sumamos la posición REAL del cuerpo de Pymunk
				# Esto asegura que donde ves el palo es donde Pymunk calcula el choque
				p = v.rotated(palo.angle) + palo.position
				vertices_mod.append((p.x, p.y))
			pygame.draw.polygon(screen, (100, 100, 100), vertices_mod)
		#------------------------------------------------------------------
		
		#print(f'{bola.position.x:.2f}  {HOYO_m*PX_M:.2f}  {bola.position.x-HOYO_m*PX_M:.2f} {bola.position.y:.2f} {bola.velocity.x:.2f} {NIVEL_DEL_SUELO-2*RADIO_PX}')
		
		#pygame.draw.line(screen,(0,0,0),xy2pixel([HOYO_m+RADIO_m*145,0]) , [WIDTH,NIVEL_DEL_SUELO], 5)
		#pygame.draw.line(screen,(0,0,0),xy2pixel([0,0]) , xy2pixel([HOYO_m-RADIO_m*145,0]), 5)

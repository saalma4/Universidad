...
# La velocidad angular se usa para mostrar visualmente la rodadura de la bola.
# En una vista cenital, el giro real 3D de una esfera se aproxima con el giro 2D
# que Pymunk ofrece alrededor del eje perpendicular a la pantalla.
...
# =============================================================================
# Datos de terreno y geometria
# =============================================================================

#Propiedades fisicas y visuales de una superficie del campo.
...
GREEN = SurfaceType("green", 0.12, (60, 150, 72), "green.png")
FAIRWAY = SurfaceType("calle", 0.25, (90, 180, 83), "fairway.png")
ROUGH = SurfaceType("hierba alta", 0.55, (37, 112, 55), "rough.png")
SAND = SurfaceType("arena", 0.90, (216, 190, 122), "sand.png")
WATER = SurfaceType("agua", 0.05, (45, 116, 190), "water.png")


#Rectangulo expresado en coordenadas fisicas, usando metros.
@dataclass(frozen=True)
class RectF:
    x: float
    y: float
...
#Zona rectangular de terreno con una superficie asociada.
@dataclass
class SurfaceZone:
    rect_m: RectF
    surface: SurfaceType
    ...
    def draw(self, screen: pygame.Surface, textures: dict[str, pygame.Surface]) -> None:
        rect_px = rect_to_px(self.rect_m)
        texture = textures.get(self.surface.texture_name or "")
        if texture is None:
            pygame.draw.rect(screen, self.surface.color, rect_px)
        else:
            tile_surface(screen, texture, rect_px)

#Obstaculo rectangular estatico construido con cuatro segmentos de Pymunk.
@dataclass
class StaticBox:
    rect_m: RectF
    color: tuple[int, int, int]
    ...
    def draw(self, screen: pygame.Surface, texture: pygame.Surface | None = None) -> None:
        rect_px = rect_to_px(self.rect_m)
        if texture is None:
            pygame.draw.rect(screen, self.color, rect_px, border_radius=7)
        else:
            tile_surface(screen, texture, rect_px)
            pygame.draw.rect(screen, (70, 42, 25), rect_px, width=2, border_radius=7)

#Informacion necesaria para construir un hoyo del minigolf.
@dataclass
class LevelData:
    name: str
...
class GolfBall:
    ...
    #Aplica un impulso en el centro de la bola.
    ...
    #Aplica resistencia a la rodadura opuesta a la velocidad lineal
    def apply_rolling_resistance(self, crr: float, dt: float) -> None:
        ...
        # En rodadura ideal v = omega R. Se usa como giro visual para que la
        # marca de la bola transmita sensacion de rodadura en la vista cenital.
...
class MinigolfGame:
    ...
    def load_level(self, index: int) -> None:
        ...
        self.golpes = 0
        self.penalties = 0
        self.message = f"Hoyo {self.level_index + 1}/{len(self.levels)}: {self.level.name}"
        self.aiming = False
    ...
    def _check_hole(self) -> None:
        ...
        if distance <= capture_radius and self.ball.speed <= HOLE_CAPTURE_SPEED:
            total = self.golpes
            self.ball.reset(self.level.start)
            self.message = f"Embocada en {total} golpes. Pulsa N para el siguiente hoyo."
    ...
    def _draw_course(self) -> None:
        for zone in self.level.zones:
            zone.draw(self.screen, self.textures)

        pygame.draw.rect(self.screen, (85, 54, 35), (0, 0, SCREEN_W, int(COURSE_H_M * PX_M)), width=8)

        wood = self.textures.get("wood.png")
        for obstacle in self.obstacles:
            obstacle.draw(self.screen, wood)
    ...
    def _draw_panel(self) -> None:
        ...
        lines = [
            f"Hoyo {self.level_index + 1}/{len(self.levels)}: {self.level.name}   |   Golpes: {self.golpes}   |   Superficie: {surface.name} (Crr={surface.crr:.2f})",
            f"Velocidad: {speed:.2f} m/s   |   R reinicia   N cambia hoyo   ESC sale",
            self.message,
        ]
...
def build_levels() -> list[LevelData]:
    ...
    level_4 = LevelData(
        ...
        zones=[
            full_green,
            SurfaceZone(RectF(0.70, 0.24, 0.60, 0.48), SAND),
            SurfaceZone(RectF(1.46, 1.25, 0.62, 0.50), SAND),
            SurfaceZone(RectF(2.42, 0.32, 0.50, 0.56), ROUGH),
            SurfaceZone(RectF(0.00, 1.78, COURSE_W_M, 0.37), FAIRWAY),
        ],
        obstacles=[
            RectF(1.18, 0.90, 0.92, 0.11),
            RectF(2.22, 1.22, 0.13, 0.62),
            RectF(2.78, 0.86, 0.14, 0.54),
        ],
    )

    level_5 = LevelData(
        name="Circuito del lago",
        start=(0.12, 0.28),
        hole=(3.18, 0.36),
        zones=[
            full_green,
            # Diseno basado en el boceto: salida arriba a la izquierda, un muro
            # inicial, bunker cercano, dos lagos grandes, hierba alta central y
            # hoyo arriba a la derecha. Las zonas se superponen sobre el green.
            SurfaceZone(RectF(0.78, 0.52, 0.42, 0.58), SAND),
            SurfaceZone(RectF(1.52, 0.02, 1.10, 0.80), WATER),
            SurfaceZone(RectF(0.25, 1.45, 0.85, 0.55), WATER),
            SurfaceZone(RectF(2.82, 0.95, 0.62, 0.52), WATER),
            SurfaceZone(RectF(1.78, 1.15, 0.65, 0.45), ROUGH),
        ],
        obstacles=[
            RectF(0.55, 0.05, 0.18, 0.78),
            RectF(1.55, 1.65, 1.95, 0.12),
        ],
    )
...
# =============================================================================
# Conversion y recursos visuales
# =============================================================================
...
#Carga texturas PNG si existen. Si falta alguna, el juego usa colores planos
def load_textures() -> dict[str, pygame.Surface]:
    textures: dict[str, pygame.Surface] = {}
    for name in ["green.png", "fairway.png", "rough.png", "sand.png", "water.png", "wood.png"]:
        path = ASSETS_DIR / name
        if path.exists():
            textures[name] = pygame.image.load(str(path)).convert()
    return textures

   
# Rellena un rectangulo repitiendo una textura.
def tile_surface(screen: pygame.Surface, texture: pygame.Surface, rect: pygame.Rect) -> None:
    tex_w, tex_h = texture.get_size()
    for y in range(rect.top, rect.bottom, tex_h):
        for x in range(rect.left, rect.right, tex_w):
            screen.blit(texture, (x, y), area=pygame.Rect(0, 0, min(tex_w, rect.right - x), min(tex_h, rect.bottom - y)))

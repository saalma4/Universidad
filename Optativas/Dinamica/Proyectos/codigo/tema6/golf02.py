...
import rozamiento_aire


############################################################
def aplicar_rodadura(body,FNorm, R, Crr=0.02, dt=1/60.0):
	"""
	Aplica un torque disipativo que simula la resistencia a la rodadura.
	No genera movimiento inverso al detenerse.
	"""
	# 1. Fuerza Normal (N) y Torque de rodadura maximo (Crr * N * R)
	# Suponemos gravedad vertical (y). Si hay pendientes, se usaria cos(theta)
	FNorm = abs(body.mass * 900) # 900 es tu gravedad actual
...
#C) Creamos la cabeza del palo
	...
	palo.velocity=Vec2d(21.165*PX_M,0)
	...
	while running:
		...
		if bola.position.y<NIVEL_DEL_SUELO-5: volando=True
		...
		for _ in range(substep):
			...
			if volando and abs(bola.angular_velocity)>0.1 and (bola.position.y>NIVEL_DEL_SUELO-2*RADIO_PX-5): 
				bola.angular_velocity*=0.95
				aplicar_rodadura(bola,bola.mass*space.gravity.y, RADIO_PX, Crr=0.12, dt=dt)
			space.step(dt)
